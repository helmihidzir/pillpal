...
import { medicationPath, medicationsPath } from "@/routes"
import type { SharedProps } from "@/types"

interface EditMedicationProps extends SharedProps {
  medication: {
    id: number
    name: string
    dosage: string | null
...
export default function EditMedication() {
  const { medication } = usePage<EditMedicationProps>().props
  ...
  return (
    <AppLayout>
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <Form
          method="patch"
          action={medicationPath(medication.id)}
          className="space-y-6"
        >
          {({ processing, errors }) => (
            <>
                  ...
                  {SCHEDULE_OPTIONS.map(({ value, label, icon: Icon }) => (
                    ...
                    >
                      <Checkbox
                        name="schedules[]"
                        value={value}
                        defaultChecked={medication.schedules.includes(value)}
                        className="sr-only"
                      />
